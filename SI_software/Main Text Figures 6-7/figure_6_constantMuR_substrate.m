...
f2 = figure('Position',[500 500 900 400]);
clf(f2);
...
aproximation = hA' > 0.1*Z_flux_free_resources1;
...
xlabel('$N_a\omega_A  \quad [\mathrm{mRNA}\cdot \mathrm{min}^{-1}]$','Interpreter','latex');
hbc=colorbar;
title('$J_A(\mu,r) \quad [adim]$','Interpreter','latex')
ylim([0 0.3]);

caxis([min(min(J_A)) max(max(J_A))]);
hold on;
grid off;
% 
% [M_flux_free,c_flux_free]=contour(...
%         X_promoter1,...
%         Y_rbs1,...
%         aproximation',...
%         1,'--','LineWidth',2);
...
% RBS ribosomal
plot(X_promoter1, KrC0*ones(size(X_promoter1(1,:))),'k--','LineWidth',1.5);
% RBS non-ribosomal
plot(X_promoter1, KnrC0*ones(size(X_promoter1(1,:))),'w--','LineWidth',1.5);
...
substrate_choice_index=1;
substrate_value=num2str(f_substrate_round(1,substrate_choice_index));
title('$\mu r \quad [molec \cdot min^{-1}]$','Interpreter','latex')
caxis([min(min(Z_flux_free_resources1)) max(max(Z_flux_free_resources1))]);
ylim([0 0.3]);
...
% saveFig(f2,'figConstant_mur_substrate',1,'/home/nobel/Sync/phd/paper/2020_Minimal_Bioarchive_Texte/images/');
