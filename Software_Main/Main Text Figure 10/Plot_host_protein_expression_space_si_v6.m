...
model_p.mu_estimated = Bremer_exp_data.mu; %Initial estimations for the function eval_host_protein_ss_v6.m
model_p.Phi_b = 0.99*ones(length(f_substrate),1)'; %Initial iteration value (based on the results obtained from Analysis_results_estim_Nr57_v6.m)
model_p.Phi_b_t = 0.83*ones(length(f_substrate),1)'; %Initial iteration value (based on the results obtained from Analysis_results_estim_Nr57_v6.m)
model_p.Phi_r_t = 0.3*ones(length(f_substrate),1)';  %Initial iteration value (based on the results obtained from Analysis_results_estim_Nr57_v6.m)
model_p.Phi_p_t = 0.55*ones(length(f_substrate),1)';  %Initial iteration value (based on the results obtained from Analysis_results_estim_Nr57_v6.m)
...
model_p.WEm_A =  1.1575; %Weight (1+1/EmA) for average non-ribosomal protein-coding genes. Change if lp_A is not average
...
for p=1:length(varying_omega_A)
    ...
    [ke,KC0_r,KC0_nr, KC0_A,mu_estimated_profile,mp_estimated,mu_r_profile,  JNr, JNnr, J_A, JWSum]  = eval_host_protein_ss_v6(proteinA_params, f_substrate); 
    ...
    estimated_specific_productivity_A = estimated_fraction_A.*mu_estimated_profile;
    ...
    BetaP_P_estimated = JNnr.* r_estimated.*nu_t_si.*model_p.dm_nr./model_p.lp_nr./model_p.Omega_nr/model_p.N_nr;
